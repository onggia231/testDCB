...
/**
 * The Check Duplication Component limit time for refund/reverse request
 * <p>
 */

@Component("cbs-check-duplicate-limit-time-refund")
@UriEndpoint(
        firstVersion = "1.0.0",
        scheme = "cbs-check-duplicate-limit-time-refund",
        title = "Check Content",
        syntax = "cbs-check-duplicate-limit-time-refund:",
        label = "cbs,endpoint",
        producerOnly = true,
        generateConfigurer = false
)

public class R_CheckRequestIdempotentLimitTimeRefundCmp extends ProcessorComponent {
...
/*                        if (String.valueOf(CBCode.OK.getCode()).equals(resultCode)){
                            CbsUtils.putValueIntoMapCheckNullValue(mapFullRequest,CbsContansts.REFERENCE_IDEMPOTENT_ID,referTransId);
